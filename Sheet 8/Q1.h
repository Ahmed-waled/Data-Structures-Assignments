...
bool isCompleteFull(queue &qq){
    return qq.rear == MAX_QUEUE_SIZE - 1 && qq.front == 0;
}

bool isVirtuallyFull(queue &qq){
    return qq.rear == MAX_QUEUE_SIZE - 1 && qq.front != 0;
}

void enqueue(queue &qq, int val){
    if(isCompleteFull(qq)){
        cout<<"Queue is full\n";
        return;
    }
    if(isVirtuallyFull(qq)){
        cout<<"Queue is virtually full\n";
        return;
    }
...
void display(queue &qq){
    ...
    while(!isEmpty(qq)){
        cout<<peak(qq)<<' '<<qq.front + 1<<' '<<qq.rear + 1<<'\n';
        dequeue(qq);
    }
}
